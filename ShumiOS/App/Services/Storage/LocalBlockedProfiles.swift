...
enum LocalBlockedProfiles {
    private static let key = "shum.local-blocked-profiles.v1"
    static func load() -> [BlockedProfileResponse] {
        ...
        for value in UserDefaults.standard.stringArray(forKey: "shum.blocked-profile-ids") ?? [] {
            if !known.contains(value.lowercased()), let id = UUID(uuidString: value) {
                profiles.append(BlockedProfileResponse(shumId: id, name: nil,
