...
final class PeopleViewModel: ObservableObject {
    ...
    func loadUserIfNeeded(shumID: String) async {
        scheduleProfileResolution(for: shumID, immediately: true)
        await profileTasks[shumID]?.value
    }

    func refreshUser(shumID: String) async {
        guard devices[shumID] != nil else { return }
        profileTasks[shumID]?.cancel()
        profileTasks[shumID] = nil
        resolutionAttempts[shumID] = 0
        suppressedProfileIDs.remove(shumID)
    ...
    func synchronizeBlockedProfiles() async {
        ...
        do {
            ...
            let serverIDs = Set(
                profiles.map { $0.shumId.uuidString.lowercased() }
            )
    ...
    func block(_ user: NearbyUser) async throws {
        ...
        if let blockSubmitter {
            ...
            blockedProfile = BlockedProfileResponse(shumId: user.id,
    ...
    func unblock(_ profile: BlockedProfileResponse) async throws {
        let generation = accountGeneration
        try await unblockSubmitter(profile.shumId)
        guard generation == accountGeneration else {
            throw CancellationError()
        }
        blockedStateRevision += 1
        blockedProfileStore.remove(profile.shumId)
    ...
    func startAdvertising(shumID: UUID) {
        bleManager.startAdvertising(id: shumID.uuidString.lowercased())
    }

    func restartAdvertising(shumID: UUID) {
        bleManager.restartAdvertising(id: shumID.uuidString.lowercased())
    }
    ...
    private func validatedUser(
        _ profile: ShumProfileResponse,
        requestedID: UUID
    ) throws -> NearbyUser {
        guard profile.shumId == requestedID else {
...
extension PeopleViewModel: BLEManagerDelegate {
    nonisolated func didReceiveProfile(id: String, epoch: UUID) {
        callbackDispatcher.dispatch { @MainActor [weak self] in
            ...
            Task { @MainActor in await self.refreshUser(shumID: id) }
        }
    }
