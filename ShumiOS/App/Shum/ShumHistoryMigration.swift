...
struct ShumLegacyArchive: Codable {
    ...
    var messages: [ShumMessage]
    static func peerID(_ contactID: String) -> PeerID { PeerID(str: "legacy-" + contactID) }
}

@MainActor
enum ShumHistoryMigration {
    static func migrate(into store: SpotchatConversationStore, url: URL,
                        readKey: () throws -> Data?) throws {
        guard store.state.legacyHistory == nil else { return }
        guard FileManager.default.fileExists(atPath: url.path) else { return }
        guard let key = try readKey(), key.count == 32 else { throw SpotchatFailure.unavailableIdentity }
        let data = try Data(contentsOf: url)
        guard data.count <= 100_000_000 else { throw SpotchatFailure.storage }
    ...
    static func live(into store: SpotchatConversationStore) throws {
