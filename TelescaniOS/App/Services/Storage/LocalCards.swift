...
final class LocalCardStore: @unchecked Sendable {
    ...
    private let keyName = "telescan.local-card.signing-key.v1"
    ...
    func snapshot(_ manifest: LocalCardManifest) -> TelescanProfileResponse {
        let body = manifest.body
        return TelescanProfileResponse(telescanId: body.id, name: body.name, username: body.username,
            bio: body.bio, photoUrl: photoURL(body.photoHash)?.absoluteString)
    }
    func profile(_ id: UUID) throws -> TelescanProfileResponse {
        try lock.withLock {
    ...
    func isBlocked(_ id: UUID) -> Bool {
        Set(defaults.stringArray(forKey: "telescan.blocked-profile-ids") ?? []).contains(id.uuidString.lowercased())
    }
    func reset() throws {
        try lock.withLock {
